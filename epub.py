...
class EpubBook:
    ...
    def __makeTitlePage(self):
        ...
        self.titlePage.html = stream.render('xhtml',
                                            doctype = 'xhtml11',
                                            drop_xml_decl = False)
    ...
    def __writeTocNCX(self):
        ...
        stream = tmpl.generate(book = self)
        fout.write(unicode(stream.render('xml')).encode('utf-8'))
        fout.close()
    ...
    def __writeItems(self):
        for item in self.getAllItems():
            ...
            if item.html:
                ...
                fout.write(unicode(item.html).encode('utf-8'))
                fout.close()
            else:
    ...
    def checkEpub(checkerPath, epubPath):
        params = ["java", "-jar", checkerPath, epubPath]
        print params
...
""" % (text, text)
    ...
    EpubBook.checkEpub('epubcheck.jar', rootDir + '.epub')
